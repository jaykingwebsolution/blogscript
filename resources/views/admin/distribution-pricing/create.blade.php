        ...
        <form method="POST" action="{{ route('admin.distribution-pricing.store') }}" class="space-y-6">
            ...
            <div class="flex justify-between items-center pt-6">
                <div class="flex items-center space-x-4">
                    ...
                    <form method="POST" action="{{ route('admin.distribution-pricing.generate-random') }}" class="inline-block">
                        @csrf
                        <button type="submit" 
                                class="px-6 py-3 bg-purple-600 text-white rounded-lg hover:bg-purple-700 transition-colors flex items-center space-x-2"
                                onclick="return confirm('Generate a random distribution plan for demo/testing purposes?')">
                            <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
